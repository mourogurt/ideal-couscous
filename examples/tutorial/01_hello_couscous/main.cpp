...
int main () {
    std::cout << std::boolalpha << couscous::check_reflected<InStructReflect>() << '\t' << couscous::check_reflected<InStructReflect>() << '\t'
              << couscous::check_reflected<NotReflected>() << '\t' << couscous::check_reflected<int>() << std::endl; //Check if type is reflected(returns boost::hana::bool_c<true/false>
    ...
    std::cout << hana::is_just(something) << '\t' << hana::is_nothing(something) << std::endl;
    constexpr auto nothing = couscous::class_name<NotReflected>();
    std::cout << hana::is_just(nothing) << '\t' << hana::is_nothing(nothing) << std::endl;
    std::cout << hana::to<const char*>(something.value()) << std::endl; //get ct-string from optional and convert to const char*
    ...
    std::cout << optional_char_str.value() << std::endl;
    std::cout << hana::is_nothing(still_nothing) << std::endl;
