...
#define STRING_MAXLEN 128                                            //Max length of a ct-string(also this parameter specifies maximum length of a var/method name)
                                                                     //Default value: 64 (if STRING_MAXLEN isn't defined). It's important to define it BEFORE include the reflection headers
#include <reflect.hpp>                                               //Ideal couscous' main header
...
struct InStructReflect {
    ...
    IN_METAINFO(InStructReflect)    //In class meta information(also it means that the struct is reflected)
...
TEMPLATE_METAINFO(OutTemplateStruct,/*nothing*/,int)                  //Spetialize Metainformation for OutTemplateStruct<int>
REFLECT_OBJ_VAR(var)
END_METAINFO
...
int main () {
    ...
    constexpr auto something = couscous::class_name<InStructReflect>(); // class_name returns boost::hana::optional of ct-string if type is reflected, otherwise it returns boost::hana::nothing
    ...
    constexpr auto optional_char_str = hana::transform(something,hana::to<const char*>); //The other way to "interact" with optionals is to use hana::transform(now it returns optional of const char*)
    constexpr auto still_nothing = hana::transform(nothing,hana::to<const char*>); //It will return boost::hana::nothing
    ...
    std::cout << "Count of all methods: " << couscous::get_opt_val(couscous::count<InStructReflect,couscous::AllMethods>())
              << "\tCount of static variables: "  << couscous::get_opt_val(couscous::count<OutStructReflect,couscous::StaticVars>()) << std::endl; //Function named "count" returns count of items in selected generator(AllMethods,StaticVars)
