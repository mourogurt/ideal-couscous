...
//Max length of a ct-string(also this parameter specifies maximum length of a var/method name).
//Default value: 64 (if STRING_MAXLEN isn't defined). It's important to define it BEFORE include the reflection headers.
...
struct InStructReflect {
    ...
    REFLECT_OBJ_VAR(var)
    //Reflect object method (first param - method name, second - method input types(nothing if method doesn't require input args)).
    REFLECT_OBJ_MTD(foo)
...
//Note: You can easily reflect method prototype and use all reflection functions which doesn't call this method directly.
...
//Note: The same rules works with static vars (You can reflect them even if they're still uninitialized).
...
//Second parameter needed to define all templetes which will be used; third parameter - spetialization for metainformation.
...
int main () {
    ...
    std::cout << std::boolalpha << "InStructReflect reflected: "
              << couscous::check_reflected<InStructReflect>() << "\tOutStructReflect reflected: "
              << couscous::check_reflected<OutStructReflect>()
              << "\nNotReflected reflected: "<< couscous::check_reflected<NotReflected>()
              << "\tint reflected: " << couscous::check_reflected<int>() << std::endl;
    //"class_name" returns boost::hana::optional of ct-string if type is reflected, otherwise it returns boost::hana::nothing.
    constexpr auto something = couscous::class_name<InStructReflect>();
    std::cout << "Optional contains something: " << hana::is_just(something)
              << "\tOptional is empty: " << hana::is_nothing(something) << std::endl;
    constexpr auto nothing = couscous::class_name<NotReflected>();
    std::cout << "Optional contains something: " << hana::is_just(nothing)
              << "\tOptional is empty: " << hana::is_nothing(nothing) << std::endl;
    //get ct-string from optional and convert to const char*
    std::cout << "Class name: " << hana::to<const char*>(something.value()) << std::endl;
    //The other way to "interact" with optionals is to use hana::transform(now it returns optional of const char*).
    constexpr auto optional_char_str = hana::transform(something,hana::to<const char*>);
    //It will return boost::hana::nothing.
    constexpr auto still_nothing = hana::transform(nothing,hana::to<const char*>);
    ...
    //Function named "count" returns count of items in selected generator(AllMethods,StaticVars).
    std::cout << "Count of all methods: " << couscous::get_opt_val(couscous::count<InStructReflect,couscous::AllMethods>())
              ...
              << couscous::get_opt_val(couscous::count<OutStructReflect,couscous::StaticVars>()) << std::endl;
    ...
    std::cout << "Count of all methods: "
              << couscous::get_opt_val(couscous::count<OutTemplateStruct<std::string>,couscous::AllMethods>()) << std::endl;
    std::cout << "Count of all variables: "
              << couscous::get_opt_val(couscous::count<OutTemplateStruct<int>,couscous::AllVars>()) << std::endl;
    return 0;
}
