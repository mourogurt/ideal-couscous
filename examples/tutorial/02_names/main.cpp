...
int main() {
    ...
    std::cout << std::boolalpha << "Equal strings: " << (BOOST_HANA_STRING("Hello couscous") == HANA_STR("Hello couscous")) << std::endl;
    ...
    std::cout << "Method name: " << couscous::get_opt_val(hana::transform(couscous::member_name<SomeStruct,couscous::AllMethods>(hana::size_c<0>),hana::to<const char*>)) << std::endl;
    hana::for_each(couscous::gen_inds_tup<decltype (couscous::get_opt_val(couscous::count<SomeStruct,couscous::AllMethods>()))>(),[](auto&& item){
        std::cout << "Method name: " << couscous::get_opt_val(hana::transform(couscous::member_name<SomeStruct,couscous::AllMethods>(item),hana::to<const char*>)) << std::endl;
    ...
    hana::for_each(inds_tup,[](auto&& item) {
        std::cout << "Index: " << item << '\t' << couscous::get_opt_val(hana::transform(couscous::member_name<SomeStruct,couscous::AllMethods>(item),hana::to<const char*>)) << std::endl;
    });
}
